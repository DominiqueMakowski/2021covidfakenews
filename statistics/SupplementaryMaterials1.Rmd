...
```{r warning=FALSE, message=FALSE}
scores <- predict(pca, names=c("Disorganized", "Paranormal_Beliefs", "Religiosity", "Negative_Affect", "Sensitivity", "Intolerance_Discomfort"))
...
```{r warning=FALSE, message=FALSE}
model_intolerance <- glmmTMB(News_Belief ~ Type / Intolerance_Discomfort + News_Familiarity + (1+Type|Participant) + (1|Dataset) + (1|News_Item), data = df_scores)
...
modelbased::estimate_link(model_intolerance, target=c("Type", "Intolerance_Discomfort")) %>%
 ggplot(aes(x=Intolerance_Discomfort, y=Predicted)) +
 geom_ribbon(aes(ymin = CI_low, ymax = CI_high, fill=Type), alpha=0.33) +
 geom_line(aes(color=Type), size = 1) +
 theme_modern() +
 ylab("Reality Belief") +
 xlab("Intolerance to Discomfort") +
 ggtitle("Effect of Intolerance to Discomfort on Reality Perception") + 
 scale_color_manual(values=colors_fakereal) +
 scale_fill_manual(values=colors_fakereal)
```
